  ...
  const initialData = async () => {
   ...
   `;
    ...
    uniqueTeams.forEach((team) => {
      ...
      // DNF racers eligible for penalty: those who DNF'd (not DNS or NS)
      const dnfRacers = teamRacers.filter((r) => !r.completed && !r.run1Dns && !r.run2Dns && !r.run1Ns);
            ...
            <div key={idx} className="text-sm border-b last:border-b-0 pb-1 last:pb-0">
              <div className="flex justify-between items-center">
                ...
                <span className="text-neutral-600">
                  {racer.hasPenalty ? <Badge variant="warning">DNF</Badge> : racer.totalTime}
                </span>
              </div>
            </div>
          ))}
